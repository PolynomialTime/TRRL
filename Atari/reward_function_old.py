...
class RwdFromRwdNet(RewardFn):
    ...
    def __init__(self, rwd_net: RewardNet, device: str = "cpu"):
        """Args:
    ...
    def __call__(
        ...
        # If shape is (N, 4, 84, 84), flatten to (N, 28224):
        if len(state_t.shape) == 4:
            # Flatten: (N, 4, 84, 84) => (N, 4 * 84 * 84)
            state_t = state_t.reshape(state_t.shape[0], -1)

        with torch.no_grad():
            reward = self.rwd_net(state_t)

        # Return a numpy array for stable-baselines to handle
